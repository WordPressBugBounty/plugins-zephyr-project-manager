...
(function ($) {
  ...
  ZephyrProjects.initialize_calendar = function (selector) {
    ...
    ZephyrProjects.getCalendarItems(function (data) {
      ...
      var options = {
        ...
        eventRender: function (event, element, view) {
          ...
          var new_description =
            ...
            event.title +
            '</strong><br/>' +
            '<strong class="zpm-calendar__description-label">Description: </strong><p class="zpm-calendar__description">' +
            event.description +
            '</p><br/>';

          if (event.original.formattedStart !== '') {
            new_description +=
              '<strong class="zpm-calendar__date-start-label">' +
              zpm_localized.strings.start_date +
              ': </strong><span class="zpm-calendar__date-start">' +
              event.original.formattedStart +
              '</span><br/>';
          }

          if (event.original.formattedDue !== '') {
            new_description +=
              '<strong class="zpm-calendar__date-start-label">' + zpm_localized.strings.due_date + ': </strong><span class="zpm-calendar__date-start">' + event.original.formattedDue + '</span><br/>';
          }

          if (event.itemType !== 'project') {
            new_description +=
              '<strong class="zpm-calendar__project-label">Project: </strong><span class="zpm-calendar__project">' +
              event.project +
              '</span><br/>' +
              '<strong class="zpm-calendar__assignee-label">Assignee: </strong>';
            var count = 0;
      ...
      $('body')
        .find(calendarSelector + ' .zpm_task_loader')
        .remove();
      ...
      ZephyrProjects.ajax(
        ...
        function (response) {
          ...
          zpm_project_doughnut_data = {
            labels: [zpm_localized.strings.completed_tasks, zpm_localized.strings.pending_tasks, zpm_localized.strings.overdue_tasks],
            datasets: [
              {
                data: [completed_projects, pending_projects, overdue_tasks],
                backgroundColor: ['#00bc8a', '#6500d8', '#e8005c'],
                borderWidth: 0,
              },
            ],
          };
  ...
  ZephyrProjects.loaderNotification = function (text) {
    ...
    var notification =
      '<div id="zpm_system_notification" class="zpm-loader-notification" role="alert"><span class="zpm-notification-icon"><i class="bi bi-arrow-clockwise"></i></span> ' + text + '</div>';
  ...
  ZephyrProjects.quick_input_modal = function (title, accept_btn, decline_btn, accept_callback, decline_callback) {
    ...
    var html =
      '<div id="' +
      modal_id +
      '" class="zpm-quick-input-modal zpm-modal"><h3 class="zpm-modal-title">' +
      title +
      '</h3>' +
      input_field +
  ...
  ZephyrProjects.confirm_modal = function (title, text, button_text, callback) {
    ...
    var html =
      "<div id='" +
      id +
      "' class='zpm-modal zephyr-modal zephyr-confirm-modal' data-modal-action='remove'><h5 class='zpm-modal-header'>" +
      title +
      "</h5><p class='zpm-modal-text'>" +
      text +
      "</p><div class='zpm-modal-buttons'><button class='zpm-modal-dismiss-error zpm_button' data-zpm-trigger='remove_modal'>" +
  ...
  ZephyrProjects.promptInput = function (options) {
    ...
    var html =
      "<div id='" +
      id +
      "' class='zpm-modal zephyr-modal zephyr-confirm-modal' data-modal-action='remove'>" +
      "<h5 class='zpm-modal-header'>" +
      options.title +
      '</h5>' +
      "<p class='zpm-modal-text'>" +
      inputHtml +
      '</p>' +
      "<div class='zpm-modal-buttons'>" +
      "<button class='zpm-modal-dismiss-error zpm_button' data-zpm-trigger='remove_modal'>" +
      zpm_localized.strings.cancel +
      '</button>' +
      "<button id='" +
      btnID +
      "' class='zpm_button zpm-submit-input-btn' data-zpm-trigger='remove_modal'>" +
      options.confirmText +
      '</button>' +
      '</div>' +
      '</div>';
  ...
  ZephyrProjects.filepicker = function (callback) {
    const $modal = ZephyrProjects.modal(
      'file-uploader-type',
      `
